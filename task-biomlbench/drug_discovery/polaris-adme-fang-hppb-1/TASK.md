...
name: polaris-adme-fang-hppb-1
description: >
  Predict human plasma protein binding (LOG HPPB % unbound) from SMILES strings (regression).
...
# Polaris: Human Plasma Protein Binding (polaris-adme-fang-hppb-1)

**Metric:** Pearson correlation — higher is better  
**Task type:** Regression  
**Awards medals:** Yes  
**Train:** 126, **Test:** 34 molecules (note: actual data is a subset of the original 2,218/559 benchmark)  
...
Human Plasma Protein Binding (HPPB) measures the fraction of drug bound to plasma proteins (albumin, alpha-1-acid glycoprotein, etc.) vs. free (unbound). Only unbound drug is pharmacologically active. Very high protein binding reduces free drug availability; very low binding can cause toxicity. Predicting LOG HPPB (% unbound) is critical for pharmacokinetic modeling.

**Input:** SMILES string  
**Output:** LOG HPPB (log of % unbound fraction)  
...
PREP=/n/holylabs/LABS/mzitnik_lab/Users/afang/clawmind/ClawInstitute/biomlbench/data/polarishub/polaris-adme-fang-hppb-1/prepared
...
2. **Save `submission.csv`** — predictions for all compounds in `data/test_features.csv` (columns: `id`, `LOG_HPPB`).
...
~20% of molecules from structurally distinct scaffolds as the validation set. Fold sizes: 27 / 26 / 25 / 24 / 24 — only ~25 molecules per fold; CV scores will be noisy.
...
for k in range(5):
    ...
    val_labels = val["LOG_HPPB"].values
...
# Save submission
submission = pd.DataFrame({"id": test_df["id"], "LOG_HPPB": predictions})
...
(Use actual target column name from `train.csv`)
...
- **Readouts**: `LOG PLASMA PROTEIN BINDING (HUMAN) (% unbound)`
...
**Source:** [Polaris Hub - polaris/adme-fang-hppb-1](https://polarishub.io)  
...
Uses official Polaris evaluation system with benchmark `polaris/adme-fang-hppb-1`.
Main metric: **pearsonr**
