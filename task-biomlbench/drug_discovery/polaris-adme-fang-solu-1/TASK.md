...
name: polaris-adme-fang-solu-1
description: >
  Predict aqueous solubility (LOG_SOLUBILITY) from drug SMILES strings (regression).
...
# Polaris: Aqueous Solubility (polaris-adme-fang-solu-1)

**Metric:** Pearson correlation — higher is better  
**Task type:** Regression  
**Awards medals:** Yes  
**Train:** 1,578, **Test:** 400 molecules  
...
Aqueous solubility is a fundamental ADME property — drugs must dissolve in water to be absorbed and distributed. Poor solubility is one of the most common reasons for drug attrition. Predicting LOG_SOLUBILITY from molecular structure helps prioritize compounds in early drug discovery.

**Input:** SMILES string  
**Output:** LOG_SOLUBILITY (log of aqueous solubility)  
...
PREP=/n/holylabs/LABS/mzitnik_lab/Users/afang/clawmind/ClawInstitute/biomlbench/data/polarishub/polaris-adme-fang-solu-1/prepared
...
2. **Save `submission.csv`** — predictions for all compounds in `data/test_features.csv` (columns: `id`, `LOG_SOLUBILITY`).
...
~20% of molecules from structurally distinct scaffolds as the validation set. Fold sizes: 331 / 314 / 312 / 311 / 310.
...
for k in range(5):
    ...
    val_labels = val["LOG_SOLUBILITY"].values
...
# Save submission
submission = pd.DataFrame({"id": test_df["id"], "LOG_SOLUBILITY": predictions})
