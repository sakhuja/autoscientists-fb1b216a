...
name: proteingym-dms-PSAE_PICP2_Tsuboyama_2023_1PSE
description: >
  Predict stability fitness of unseen multi-substitution variants of Photosystem I reaction center
  subunit IV. Maximize Spearman correlation using 5-fold CV across 3 split strategies.
---
...
- **Multi-substitution:** Sequences may have multiple mutations from wild-type — model must handle variable-length changes.
- **No test leakage:** Train only on other folds.
- **Compute:** See `program_biomlbench.md` for wall-clock limit and GPU/CPU settings.

---

# ProteinGym DMS: PSAE_PICP2_Tsuboyama_2023_1PSE

**Metric:** Spearman correlation — higher is better  
**Difficulty:** Medium  
**Dataset size:** 1,579 sequences (multi-substitution variants)  
...
Deep mutational scanning dataset measuring **stability** fitness for **multi-substitution** variants of Photosystem I reaction center subunit IV from *Psychromonas ingrahamii* (Uniprot: PSAE_PICP2). Multi-substitution variants are harder to model than single-substitution due to combinatorial sequence space.

**Input:** Amino acid sequence of the variant  
**Output:** Three predicted fitness scores (one per CV split strategy)  
**Dataset:** 1,579 sequences with measured stability scores
...
# Option 1: Use biomlbench prepare pipeline
$PYTHON -c "
import sys; sys.path.insert(0, '$BIOMLBENCH')
from biomlbench.data_sources.proteingym import ProteinGymDMSDataSource
from pathlib import Path
src = ProteinGymDMSDataSource('PSAE_PICP2_Tsuboyama_2023_1PSE')
raw = Path('data/raw'); raw.mkdir(parents=True, exist_ok=True)
src.download(raw)
"

# Option 2: Copy from biomlbench prepared data if available
PREP=/n/holylabs/LABS/mzitnik_lab/Users/afang/clawmind/ClawInstitute/biomlbench/data/proteingym-dms/PSAE_PICP2_Tsuboyama_2023_1PSE/prepared/public
cp $PREP/data.csv data/ 2>/dev/null || echo "Prepared data not found, run biomlbench prepare"
```
...
# Proteingym-DMS dataset: PSAE_PICP2_Tsuboyama_2023_1PSE

## Description

This dataset is part of the ProteinGym DMS benchmark, which contains deep mutational scanning datasets that measure
protein fitness (in different contexts) for sequence variants of a wide range of proteins. This dataset contains
multi-substitution variants for the protein Photosystem I reaction center subunit IV from the organism Picosynechococcus. This protein has Uniprot ID: PSAE_PICP2. 
...
The objective of this benchmark is to train a model that can predict the fitness of unseen single-substitution sequence variants of Photosystem I reaction center subunit IV.
