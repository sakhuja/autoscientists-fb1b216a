...
name: proteingym-dms-SBI_STAAM_Tsuboyama_2023_2JVG
description: >
  Predict stability fitness of unseen single-substitution variants of Immunoglobulin-binding protein Sbi
  from S. aureus. Maximize Spearman correlation using 5-fold CV across 3 split strategies.
---
...
Deep mutational scanning dataset measuring **stability** fitness for single-substitution variants of Immunoglobulin-binding protein Sbi from *Staphylococcus aureus* (Uniprot: SBI_STAAM). Predict fitness of unseen variants using 5-fold CV.

**Input:** Amino acid sequence of the variant  
**Output:** Three predicted fitness scores (one per CV split strategy)  
**Dataset:** 1,025 sequences with measured stability scores
...
src = ProteinGymDMSDataSource('SBI_STAAM_Tsuboyama_2023_2JVG')
raw = Path('data/raw'); raw.mkdir(parents=True, exist_ok=True)
src.download(raw)
# Then run prepare
exec(open('$BIOMLBENCH/biomlbench/tasks/proteingym_dms/SBI_STAAM_Tsuboyama_2023_2JVG/prepare.py').read())
"
```

Alternatively, copy from biomlbench prepared data:
```bash
PREP=/n/holylabs/LABS/mzitnik_lab/Users/afang/clawmind/ClawInstitute/biomlbench/data/proteingym-dms/SBI_STAAM_Tsuboyama_2023_2JVG/prepared/public
cp $PREP/data.csv data/
cp $PREP/sample_submission.csv data/
```
...
# Proteingym-DMS dataset: SBI_STAAM_Tsuboyama_2023_2JVG

## Description

This dataset is part of the ProteinGym DMS benchmark, which contains deep mutational scanning datasets that measure
protein fitness (in different contexts) for sequence variants of a wide range of proteins. This dataset contains
single-substitution variants for the protein Immunoglobulin-binding protein Sbi from the organism Staphylococcus aureus. This protein has Uniprot ID: SBI_STAAM. 
...
The objective of this benchmark is to train a model that can predict the fitness of unseen single-substitution sequence variants of Immunoglobulin-binding protein Sbi.
