...
name: proteingym-dms-SPIKE_SARS2_Starr_2020_binding
description: >
  Predict fitness of unseen single-substitution variants of SARS-CoV-2 Spike RBD (ACE2 binding).
  Maximize Spearman correlation using 5-fold cross-validation across 3 split strategies.
---
...
- **Working directory:** Run from this task directory so `data/` paths resolve correctly.
- **No test leakage:** Models must be trained only on sequences from other folds; never use fold's fitness scores during training of that fold's model.
- **Copy `train.py` per experiment**, do not modify in place.
...
Deep mutational scanning dataset measuring ACE2 binding fitness for SARS-CoV-2 Spike RBD variants. Predict the fitness of unseen **single-substitution** sequence variants using 5-fold cross-validation.

**Input:** Amino acid sequence of the variant  
**Output:** Three predicted fitness scores (one per CV split strategy)  
**Dataset:** 3,802 sequences with measured fitness scores  
**Source publication:** "Deep Mutational Scanning of SARS-CoV-2 Receptor Binding Domain Reveals Constraints on Folding and ACE2 Binding" (DOI: 10.1016/j.cell.2020.08.012)
...
```
data/
└── data.csv          # sequences, fitness scores, fold assignments
...
from biomlbench.tasks.proteingym_dms.SPIKE_SARS2_Starr_2020_binding.prepare import prepare
from pathlib import Path
import os
# Download raw data first
from biomlbench.data_sources.proteingym import ProteinGymDMSDataSource
src = ProteinGymDMSDataSource('SPIKE_SARS2_Starr_2020_binding')
raw = Path('data/raw'); raw.mkdir(parents=True, exist_ok=True)
src.download(raw)
prepare(raw, Path('data'), Path('data'))
print('Data ready in data/')
"
```

Alternatively, copy directly from biomlbench prepared data if available:
```bash
BIOMLBENCH_PREPARED=/n/holylabs/LABS/mzitnik_lab/Users/afang/clawmind/ClawInstitute/biomlbench
cp $BIOMLBENCH_PREPARED/data/proteingym-dms/SPIKE_SARS2_Starr_2020_binding/prepared/public/data.csv data/
cp $BIOMLBENCH_PREPARED/data/proteingym-dms/SPIKE_SARS2_Starr_2020_binding/prepared/public/sample_submission.csv data/
```

### Data format (`data/data.csv`)
- `id`: sequence index
- `sequence`: amino acid sequence of the variant
- `fitness_score`: measured ACE2 binding fitness (target variable)
- `fold_random_5`: fold assignment (0–4) for random 5-fold CV
- `fold_modulo_5`: fold assignment (0–4) for modulo 5-fold CV
- `fold_contiguous_5`: fold assignment (0–4) for contiguous 5-fold CV
...
single-substitution variants for the protein Spike RBD from the organism Severe acute respiratory syndrome coronavirus 2 (2019-nCoV) (SARS-CoV-2). This protein has Uniprot ID: SPIKE_SARS2. 
